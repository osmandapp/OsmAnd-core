...
struct TransportSegmentsComparator
	: public std::binary_function<SHARED_PTR<TransportRouteSegment>&,
								  SHARED_PTR<TransportRouteSegment>&, bool> {
	TransportSegmentsComparator() {}
	bool operator()(const SHARED_PTR<TransportRouteSegment>& lhs,
					const SHARED_PTR<TransportRouteSegment>& rhs) const {
		int cmp;
...
bool TransportRoutePlanner::includeRoute(
	SHARED_PTR<TransportRouteResult>& fastRoute,
...
vector<SHARED_PTR<TransportRouteResult>> TransportRoutePlanner::prepareResults(
	SHARED_PTR<TransportRoutingContext>& ctx,
	vector<SHARED_PTR<TransportRouteSegment>>& results) {
	sort(results.begin(), results.end(),
		 [](const SHARED_PTR<TransportRouteSegment>& lhs,
			const SHARED_PTR<TransportRouteSegment>& rhs) {
	...
	OsmAnd::LogPrintf(OsmAnd::LogSeverityLevel::Info,
					  ...
					  results.size(), ctx->visitedRoutesCount,
					  ctx->visitedStops, ctx->quadTree.size(), ctx->loadedWays,
					  ctx->wrongLoadedWays);

	for (SHARED_PTR<TransportRouteSegment>& res : results) {
		if (ctx->calculationProgress.get() &&
			ctx->calculationProgress->isCancelled()) {
			return vector<SHARED_PTR<TransportRouteResult>>();
		}
		SHARED_PTR<TransportRouteResult> route =
			make_shared<TransportRouteResult>(ctx.get());
		route->routeTime = res->distFromStart;
		route->finishWalkDist = res->walkDist;
		SHARED_PTR<TransportRouteSegment> p = res;
		while (p != nullptr) {
			if (ctx->calculationProgress != nullptr &&
				ctx->calculationProgress->isCancelled()) {
				return vector<SHARED_PTR<TransportRouteResult>>();
			}
			if (p->parentRoute != nullptr) {
				SHARED_PTR<TransportRouteResultSegment> sg =
					make_shared<TransportRouteResultSegment>();
...
vector<SHARED_PTR<TransportRouteResult>>
TransportRoutePlanner::buildTransportRoute(
	SHARED_PTR<TransportRoutingContext>& ctx) {
	...
	for (SHARED_PTR<TransportRouteSegment>& r : startStops) {
		r->walkDist = getDistance(r->getLocationLat(), r->getLocationLon(),
								  ctx->startLat, ctx->startLon);
	...
	double maxTravelTimeCmpToWalk =
		getDistance(ctx->startLat, ctx->startLon, ctx->endLat, ctx->endLon) /
			ctx->cfg->walkSpeed -
	...
	while (queue.size() > 0) {
		...
		if (ctx->visitedSegments.find(segment->getId()) !=
			ctx->visitedSegments.end()) {
			ex = ctx->visitedSegments.find(segment->getId())->second;
			if (ex->distFromStart > segment->distFromStart) {
				OsmAnd::LogPrintf(
					OsmAnd::LogSeverityLevel::Error, "%.1f (%s) > %.1f (%s)",
					ex->distFromStart, ex->to_string().c_str(),
		...
		double travelTime = 0;
		const float routeTravelSpeed =
			ctx->cfg->getSpeedByRouteType(segment->road->type);

		if (routeTravelSpeed == 0) {
			continue;
		}
		SHARED_PTR<TransportStop> prevStop =
			segment->getStop(segment->segStart);
		vector<SHARED_PTR<TransportRouteSegment>> sgms;

		for (int32_t ind = 1 + segment->segStart; ind < segment->getLength();
			 ind++) {
			if (ctx->calculationProgress != nullptr &&
				ctx->calculationProgress->isCancelled()) {
				return vector<SHARED_PTR<TransportRouteResult>>();
			...
			double segmentDist =
				getDistance(prevStop->lat, prevStop->lon, stop->lat, stop->lon);
			...
			if (ctx->cfg->useSchedule) {
				...
				travelTime +=
					ctx->cfg->stopTime + segmentDist / routeTravelSpeed;
			}
			...
			for (SHARED_PTR<TransportRouteSegment>& sgm : sgms) {
				...
				SHARED_PTR<TransportRouteSegment> nextSegment =
					make_shared<TransportRouteSegment>(sgm);
				nextSegment->parentRoute = segment;
				nextSegment->parentStop = ind;
				nextSegment->walkDist = getDistance(
					nextSegment->getLocationLat(),
					nextSegment->getLocationLon(), stop->lat, stop->lon);
				...
				nextSegment->distFromStart =
					segment->distFromStart + travelTime + walkTime;
				if (ctx->cfg->useSchedule) {
					int tm =
						(sgm->departureTime - ctx->cfg->scheduleTimeOfDay) * 10;
			...
			SHARED_PTR<TransportRouteSegment> finalSegment =
				endSegments[segmentId];
			double distToEnd =
				getDistance(stop->lat, stop->lon, ctx->endLat, ctx->endLon);
		...
		if (finish != nullptr) {
			...
			if (finish->distFromStart <
					finishTime + ctx->cfg->finishTimeSeconds &&
				(finish->distFromStart < maxTravelTimeCmpToWalk ||
				 results.size() == 0)) {
...
void TransportRoutePlanner::updateCalculationProgress(
	SHARED_PTR<TransportRoutingContext>& ctx,
	...
	if (ctx->calculationProgress.get()) {
		...
		if (queue.size() > 0) {
			...
			ctx->calculationProgress->distanceFromBegin =
				(int64_t)fmax(peek->distFromStart,
							  ctx->calculationProgress->distanceFromBegin);
		}
	}
}
