    ...
    const char* appFontsPath /* = nullptr */)
{
    ...
    {
        gdalCacheMaxMB = qMax(64,
            static_cast<int>(usablePhysicalRAM / 20 / (1024 * 1024)));
    }
    char gdalCacheMax[16];
    snprintf(gdalCacheMax, sizeof(gdalCacheMax), "%d", gdalCacheMaxMB);
    CPLSetConfigOption("GDAL_CACHEMAX", gdalCacheMax);
    GDALAllRegister();
    if (!SKIA::initialize())
        return 0;
    if (!ICU::initialize())
