...
QByteArray OsmAnd::Utilities::readEntireFile(const QString& filename)
{
    QFile file(filename);
...
QByteArray OsmAnd::Utilities::readPartOfFile(const QString& filename, const size_t offset, const size_t length)
{
    QFile file(filename);
    if (!file.open(QIODevice::ReadOnly))
        return QByteArray();
