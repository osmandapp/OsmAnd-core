    ...
    bool coastlineOnly /*= false*/)
{
    ...
    for (const auto& obfReader : constOf(obfReaders))
    {
        ...
        if (obfInfo->isBasemapWithCoastlines)
        {
            ...
            {
                const auto roundZoom = static_cast<ZoomLevel>(ObfMapSectionLevel::MaxBasemapZoomLevel);
                const auto& roundedBBox31 = Utilities::roundBoundingBox31(*bbox31_, roundZoom);
                bbox31 = &roundedBBox31;
            }
        }
