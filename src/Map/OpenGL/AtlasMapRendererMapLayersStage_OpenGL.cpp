    ...
    QVector<RasterLayerTileProgram>& outRasterLayerTilePrograms)
{
    ...
    if (!outRasterLayerTileProgram.id.isValid())
    {
        ...
        fragmentShader.append(QStringLiteral(
            ...
            "}                                                                                                                  ""\n"
            "                                                                                                                   ""\n"
            "void getTextureColor(in vec4 texCoordsOffsetAndScale, in vec4 transPhase, in float texSize,                        ""\n"
        ...
        const auto& dynamicsInFragmentShader = QStringLiteral(
            ...
            "        float halfTexelSize = texSize / 2.0;                                                                       ""\n"
            ...
            "        float factor = transPhase.y / (v2f_metersPerUnit * 250.0);                                                 ""\n"
            "        vec2 texShift = rightWind * texCoordsOffsetAndScale.zw * factor;                                           ""\n"
            "        if (transPhase.z >= 0.0)                                                                                   ""\n"
            "        {                                                                                                          ""\n"
            //           Animate wind particles
            "            float seed = floor(transPhase.z) / 10.0;                                                               ""\n"
            "            float shift = floor(texCoordsOffsetAndScale.y * 10.0) / 100.0;                                         ""\n"
            "            shift += floor(texCoordsOffsetAndScale.x * 10.0) / 1000.0;                                             ""\n"
            "            float phase = fract(transPhase.z);                                                                     ""\n"
            "            vec2 tileSize = transPhase.w / texCoordsOffsetAndScale.zw;                                             ""\n"
            "            rightCoords.x -= 0.5;                                                                                  ""\n"
            "            vec2 leftWind;                                                                                         ""\n"
            "            getWind(sampler, rightCoords, leftWind);                                                               ""\n"
            "            vec2 windVector = mix(leftWind, rightWind, transPhase.x);                                              ""\n"
            ...
            "        rightCoords.x += 0.5;                                                                                      ""\n"
            "        leftCoords -= texShift * transPhase.x;                                                                     ""\n"
            "        leftCoords.x = clamp(leftCoords.x, halfTexelSize, 0.5 - halfTexelSize);                                    ""\n"
            "        leftCoords.y = clamp(leftCoords.y, halfTexelSize, 0.5 - halfTexelSize);                                    ""\n"
            "        rightCoords += texShift * (1.0 - transPhase.x);                                                            ""\n"
            ...
            "        lowp vec4 texColor = mix(texColorLeft, texColorRight, clamp(transPhase.x, 0.0, 1.0));                      ""\n"
