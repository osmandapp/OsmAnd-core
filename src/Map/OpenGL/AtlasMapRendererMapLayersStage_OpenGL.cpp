    ...
    RasterLayerTileProgram& outRasterLayerTileProgram)
{
    ...
    const auto& vertexShader = QString::fromLatin1(
        ...
        "    float heixMargin = 0.5 / (%HeixelsPerTileSide%.0 + 3.0);                                                       ""\n"
        "    vec2 minCoords = vec2(heixMargin, heixMargin);                                                                 ""\n"
        "    vec2 maxCoords = vec2(1.0, 1.0) - minCoords;                                                                   ""\n"
        "    float blHeixel = SAMPLE_TEXTURE_2D(param_vs_elevation_dataSampler, clamp(heixCoords, minCoords, maxCoords)).r; ""\n"
        "    heixCoords.x += heixelSize;                                                                                    ""\n"
        "    float brHeixel = SAMPLE_TEXTURE_2D(param_vs_elevation_dataSampler, clamp(heixCoords, minCoords, maxCoords)).r; ""\n"
        "    heixCoords.y += heixelSize;                                                                                    ""\n"
        "    float trHeixel = SAMPLE_TEXTURE_2D(param_vs_elevation_dataSampler, clamp(heixCoords, minCoords, maxCoords)).r; ""\n"
        "    heixCoords.x -= heixelSize;                                                                                    ""\n"
        "    float tlHeixel = SAMPLE_TEXTURE_2D(param_vs_elevation_dataSampler, clamp(heixCoords, minCoords, maxCoords)).r; ""\n"
