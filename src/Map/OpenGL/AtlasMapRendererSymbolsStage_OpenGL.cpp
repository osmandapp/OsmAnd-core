...
bool OsmAnd::AtlasMapRendererSymbolsStage_OpenGL::initializeBillboardRaster()
{
    ...
    const QString vertexShader = QLatin1String(
        ...
        "    highp ivec2 offsetFromTarget31 = ivec2(0);                                                                          ""\n"
        "    highp ivec2 offsetFromTarget31_base = param_vs_position31 - param_vs_target31.xy;                                   ""\n"
        "    highp ivec2 offsetFromTarget31_negative = ivec2(param_vs_position31.x - 2147483647, param_vs_position31.y) - param_vs_target31.xy;                                                                       ""\n"
        "    highp ivec2 offsetFromTarget31_positive = param_vs_position31 - ivec2(param_vs_target31.x - 2147483647, param_vs_target31.y);                                                                                                ""\n"

        "    if (abs(offsetFromTarget31_base.x) < abs(offsetFromTarget31_negative.x) && abs(offsetFromTarget31_base.x) < abs(offsetFromTarget31_positive.x))                                                                     ""\n"
        "    {                                                                                                              ""\n"
        "        offsetFromTarget31 = offsetFromTarget31_base;                                                              ""\n"
        "    }                                                                                                              ""\n"
        "    else if (abs(offsetFromTarget31_negative.x) < abs(offsetFromTarget31_positive.x))                              ""\n"
        "    {                                                                                                              ""\n"
        "        offsetFromTarget31 = offsetFromTarget31_negative;                                                              ""\n"
        "    }                                                                                                              ""\n"
        "    else                                                                                                           ""\n"
        "    {                                                                                                              ""\n"
        "        offsetFromTarget31 = offsetFromTarget31_positive;                                                              ""\n"
        "    }                                                                                                              ""\n"
        "                                                                                                                   ""\n"
