...
OsmAnd::MapStyle::MapStyle( MapStyles* owner, const QString& embeddedResourceName )
    : owner(owner)
...
OsmAnd::MapStyle::MapStyle( MapStyles* owner, const QFileInfo& externalStyleFile )
    : owner(owner)
    , _firstNonBuiltinValueDefinitionIndex(0)
    , _stringsIdBase(0)
    , resourceName(_resourceName)
...
bool OsmAnd::MapStyle::parseMetadata()
{
...
bool OsmAnd::MapStyle::parseMetadata( QXmlStreamReader& xmlReader )
{
...
bool OsmAnd::MapStyle::resolveDependencies()
{
...
bool OsmAnd::MapStyle::parse( QXmlStreamReader& xmlReader )
{
    ...
    struct Lexeme
    {
        ...
        Lexeme(Type type_, MapStyle* style_)
            : type(type_)
            , style(style_)
        {}
    ...
    {
        Rule(std::shared_ptr<MapStyleRule> rule_, MapStyle* style_)
    ...
    while (!xmlReader.atEnd() && !xmlReader.hasError())
    {
        ...
        if (xmlReader.isStartElement())
        {
            ...
            else if(tagName == "renderingAttribute")
            {
                ...
                std::shared_ptr<MapStyleRule> attribute(new MapStyleRule(this, QHash< QString, QString >()));
            ...
            else if(tagName == "filter")
            {
                ...
                std::shared_ptr<MapStyleRule> rule(new MapStyleRule(this, ruleAttributes));
            ...
            else if(tagName == "groupFilter")
            {
                ...
                std::shared_ptr<MapStyleRule> rule(new MapStyleRule(this, attributes));
...
void OsmAnd::MapStyle::registerBuiltinValue( const std::shared_ptr<ValueDefinition>& valueDefinition )
{
...
std::shared_ptr<OsmAnd::MapStyle::ValueDefinition> OsmAnd::MapStyle::registerValue( ValueDefinition* pValueDefinition )
{
...
void OsmAnd::MapStyle::registerBuiltinValueDefinitions()
{
    ...
    registerBuiltinValue(builtinValueDefinitions.OUTPUT_NAME_TAG2);
    ...
    registerBuiltinValue(builtinValueDefinitions.OUTPUT_ICON_ORDER);
...
bool OsmAnd::MapStyle::resolveConstantValue( const QString& name, QString& value )
{
...
QString OsmAnd::MapStyle::obtainValue( const QString& input )
{
    QString output;
...
QMap< uint64_t, std::shared_ptr<OsmAnd::MapStyleRule> >& OsmAnd::MapStyle::obtainRules( RulesetType type )
{
    switch (type)
    {
    case OsmAnd::MapStyle::Point:
        return _pointRules;
    case OsmAnd::MapStyle::Line:
        return _lineRules;
    case OsmAnd::MapStyle::Polygon:
        return _polygonRules;
    case OsmAnd::MapStyle::Text:
        return _textRules;
    case OsmAnd::MapStyle::Order:
        return _orderRules;
    }
...
const QMap< uint64_t, std::shared_ptr<OsmAnd::MapStyleRule> >& OsmAnd::MapStyle::obtainRules( RulesetType type ) const
{
    switch (type)
    {
    case OsmAnd::MapStyle::Point:
...
void OsmAnd::MapStyle::dump( RulesetType type, const QString& prefix /*= QString()*/ ) const
{
...
bool OsmAnd::MapStyle::registerRule( RulesetType type, const std::shared_ptr<MapStyleRule>& rule )
{
    bool ok;

    auto tag = rule->getIntegerAttribute(MapStyle::builtinValueDefinitions.INPUT_TAG->name, &ok);
    if(!ok)
    {
    ...
    auto value = rule->getIntegerAttribute(MapStyle::builtinValueDefinitions.INPUT_VALUE->name, &ok);
    if(!ok)
    {
        OsmAnd::LogPrintf(OsmAnd::LogSeverityLevel::Error, "Attribute tag should be specified for root filter");
        return false;
...
std::shared_ptr<OsmAnd::MapStyleRule> OsmAnd::MapStyle::createTagValueRootWrapperRule( uint64_t id, const std::shared_ptr<MapStyleRule>& rule )
{
...
uint32_t OsmAnd::MapStyle::getTagStringId( uint64_t ruleId ) const
{
    return ruleId >> RuleIdTagShift;
}

uint32_t OsmAnd::MapStyle::getValueStringId( uint64_t ruleId ) const
{
    return ruleId & ((1ull << RuleIdTagShift) - 1);
}

const QString& OsmAnd::MapStyle::getTagString( uint64_t ruleId ) const
{
    return lookupStringValue(getTagStringId(ruleId));
}

const QString& OsmAnd::MapStyle::getValueString( uint64_t ruleId ) const
{
    return lookupStringValue(getValueStringId(ruleId));
}

const QString& OsmAnd::MapStyle::lookupStringValue( uint32_t id ) const
{
...
bool OsmAnd::MapStyle::lookupStringId( const QString& value, uint32_t& id ) const
{
...
uint32_t OsmAnd::MapStyle::lookupStringId( const QString& value )
{
    uint32_t id;
...
uint32_t OsmAnd::MapStyle::registerString( const QString& value )
{
...
bool OsmAnd::MapStyle::resolveValueDefinition( const QString& name, std::shared_ptr<ValueDefinition>& outDefinition ) const
{
...
bool OsmAnd::MapStyle::mergeInherited()
{
    if(!_parent)
        return true;
...
bool OsmAnd::MapStyle::mergeInheritedRules( RulesetType type )
{
    if(!_parent)
        return true;
...
bool OsmAnd::MapStyle::mergeInheritedAttributes()
{
    if(!_parent)
        return true;
...
uint64_t OsmAnd::MapStyle::encodeRuleId( uint32_t tag, uint32_t value )
{
    return (static_cast<uint64_t>(tag) << RuleIdTagShift) | value;
}

bool OsmAnd::MapStyle::resolveAttribute( const QString& name, std::shared_ptr<MapStyleRule>& outAttribute ) const
{
...
OsmAnd::MapStyle::ValueDefinition::ValueDefinition( ValueDefinition::Type type_, ValueDefinition::DataType dataType_, const QString& name_ )
    : type(type_)
    , dataType(dataType_)
    , name(name_)
{
...
OsmAnd::MapStyle::ValueDefinition::~ValueDefinition()
{
}

OsmAnd::MapStyle::ConfigurableInputValue::ConfigurableInputValue( ValueDefinition::DataType type_, const QString& name_, const QString& title_, const QString& description_, const QStringList& possibleValues_ )
...
OsmAnd::MapStyle::ConfigurableInputValue::~ConfigurableInputValue()
{
}

const OsmAnd::MapStyle::_builtinValueDefinitions OsmAnd::MapStyle::builtinValueDefinitions;

#define DECLARE_BUILTIN_VALUEDEF(varname, type, dataType, name) \
    varname(new OsmAnd::MapStyle::ValueDefinition( \
        OsmAnd::MapStyle::ValueDefinition::type, \
        OsmAnd::MapStyle::ValueDefinition::dataType, \
        name \
    ))

OsmAnd::MapStyle::_builtinValueDefinitions::_builtinValueDefinitions()
