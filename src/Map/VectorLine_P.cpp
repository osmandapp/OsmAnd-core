...
std::shared_ptr<OsmAnd::VectorLine::SymbolsGroup> OsmAnd::VectorLine_P::inflateSymbolsGroup() const
{
    ...
    if (!_points.empty())
    {
        ...
        //vectorLine->primitiveType = VectorMapSymbol::PrimitiveType::LineLoop;
        ...
        if (pointsCount >= 2) {
            ...
            double nx = owner->lineWidth * sin(ntan) / 2;
            double ny = owner->lineWidth * cos(ntan) / 2;
            
            double nx1 = owner->lineWidth * sin(ntan + M_PI_4) / 2;
            double ny1 = owner->lineWidth * cos(ntan + M_PI_4) / 2;
            double nx2 = owner->lineWidth * sin(ntan - M_PI_4) / 2;
            double ny2 = owner->lineWidth * cos(ntan - M_PI_4) / 2;
            ...
            for (auto pointIdx = 0u; pointIdx < pointsCount; pointIdx++)
            {
                ...
                if(pointIdx > 0 && pointIdx < pointsCount - 1) {
                    ...
                    nx = owner->lineWidth * sin(ntan) / 2;
                    ny = owner->lineWidth * cos(ntan) / 2;
                    
                    nx1 = owner->lineWidth * sin(ntan + M_PI_4) / 2;
                    ny1 = owner->lineWidth * cos(ntan + M_PI_4) / 2;
                    nx2 = owner->lineWidth * sin(ntan - M_PI_4) / 2;
                    ny2 = owner->lineWidth * cos(ntan - M_PI_4) / 2;
