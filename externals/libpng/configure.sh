...
if [ ! -f "$SRCLOC/upstream.pack" ]; then
	...
	curl -L http://sourceforge.net/projects/libpng/files/libpng15/older-releases/1.5.13/libpng-1.5.13.tar.bz2/download > "$SRCLOC/upstream.pack" || { echo "Failure" 1>&2; exit; }
fi
