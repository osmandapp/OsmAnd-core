...
set compiler=%2
set targetArch=%3
if not "%targetOS%"=="winrt" (
	echo 'winrt' is the only supported target, while '%targetOS%' was specified
	exit /B 1
)
set validCompiler=0
if "%compiler%"=="msvc" (
...
if not "%validArch%"=="1" (
	echo 'i686', 'amd64' and 'arm' are the only supported target architectures, while '%targetArch%' was specified
	exit /B 1
)
...
set "STATIC_BUILD_PATH=%~dp0upstream.patched.winrt.%compiler%-%targetArch%.static"
if not exist "%STATIC_BUILD_PATH%" (
	mkdir "%STATIC_BUILD_PATH%"
	xcopy "%~dp0upstream.patched" "%STATIC_BUILD_PATH%" /E /Q
	pushd %STATIC_BUILD_PATH% && (cmd /C "configure.bat -static %QTBASE_CONFIGURATION% -xplatform winrt-%1-msvc2013 -prefix %STATIC_BUILD_PATH%" & popd)
	if %ERRORLEVEL% neq 0 (
